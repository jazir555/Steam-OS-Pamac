#!/bin/bash
# Steam Deck Pamac End-to-End Test
# Tests pamac functionality: search, install, uninstall, desktop export, icon integration
# Runs from WSL against the Steam Deck via SSH
set -euo pipefail
...
container_exec() { ssh_check "podman exec -i -u 0 '$CONTAINER_NAME' bash -c '${1//\'/\\\'}' 2>&1"; }
container_user_exec() { ssh_check "podman exec -i -u deck '$CONTAINER_NAME' bash -c '${1//\'/\\\'}' 2>&1"; }
distrobox_exec() { 
    local cmd="${1//\'/\\\'}"
    local timeout_sec="${2:-}"
...
distrobox_exec_quiet() {
    local cmd="${1//\'/\\\'}"
    local timeout_sec="${2:-}"
    if [[ -n "$timeout_sec" ]]; then
        ssh_check "timeout $timeout_sec distrobox-enter '$CONTAINER_NAME' -- bash -c '$cmd'" 2>/dev/null
    else
        ssh_check "distrobox-enter '$CONTAINER_NAME' -- bash -c '$cmd'" 2>/dev/null
    fi
}
...
cleanup_package() {
    ...
    distrobox_exec "pamac remove --no-confirm $pkg 2>/dev/null" || true
    distrobox_exec "pamac remove --no-confirm --unneeded $pkg 2>/dev/null" || true
    container_exec "pacman -Qs '^$pkg$' >/dev/null 2>&1 && pacman -Rdd --noconfirm '$pkg' 2>/dev/null" || true
}
...
check_desktop_file_format() {
    ...
    if ! ssh_check "grep -q '^Exec=' '$f' 2>/dev/null"; then
        fail "$desc: No Exec line found"
        return 1
    fi

    if ! ssh_check "grep '^Exec=' '$f' 2>/dev/null | head -1 | grep -q 'pamac-manager-wrapper'"; then
        fail "$desc: Exec does not use pamac-manager-wrapper"
        failures=$((failures + 1))
    fi

    if ! ssh_check "grep -q '^X-SteamOS-Pamac-Managed=true' '$f' 2>/dev/null"; then
        fail "$desc: Missing X-SteamOS-Pamac-Managed marker"
        failures=$((failures + 1))
    fi

    if ! ssh_check "grep -q 'X-SteamOS-Pamac-Container=${CONTAINER_NAME}' '$f' 2>/dev/null"; then
...
test_prerequisites() {
    ...
    if ssh_check "podman exec -i -u 0 '$CONTAINER_NAME' bash -c 'echo ok' 2>/dev/null | grep -q ok"; then
    ...
    if distrobox_exec "command -v pamac" >/dev/null 2>&1; then
    ...
    if distrobox_exec "command -v pamac-manager" >/dev/null 2>&1; then
    ...
    if distrobox_exec_quiet "grep -q EnableAUR /etc/pamac.conf"; then
    ...
    local ver
    ver=$(distrobox_exec "pamac --version 2>/dev/null | head -1" || echo "")
    if [[ -n "$ver" ]]; then
...
test_search() {
    ...
    local search_out
    search_out=$(distrobox_exec "pamac search '$TEST_PACKAGE_AUR' 2>/dev/null" 30 || true)
    ...
    local aur_search
    aur_search=$(distrobox_exec "pamac search --aur '$TEST_PACKAGE_AUR' 2>/dev/null" 30 || true)
    ...
    if distrobox_exec "pamac info '$TEST_PACKAGE_AUR' 2>/dev/null" | grep -qi "Repository.*AUR"; then
...
test_install() {
    ...
    local install_out
    install_out=$(timeout 120 distrobox_exec "pamac install --no-confirm '$TEST_PACKAGE_AUR' 2>&1" || true)
    echo "$install_out" | tail -10 >> "$TEST_LOG"

    if container_exec "pacman -Qs '^$TEST_PACKAGE_AUR$' >/dev/null 2>&1"; then
        pass "Package '$TEST_PACKAGE_AUR' installed successfully via pamac"
    else
        local pacman_out
        pacman_out=$(container_exec "pacman -Qs '$TEST_PACKAGE_AUR'" 2>&1)
        if echo "$install_out" | grep -qi "error: target not found\|nothing to do\|already installed"; then
            # Package may already be installed from a previous test
            if container_exec "pacman -Qs '$TEST_PACKAGE_AUR' >/dev/null 2>&1"; then
                pass "Package '$TEST_PACKAGE_AUR' was already installed"
            else
                fail "Package '$TEST_PACKAGE_AUR' not found after install attempt (output: ${install_out:0:300})"
            fi
        else
    ...
    if container_user_exec "command -v '$TEST_PACKAGE_AUR' 2>/dev/null" | grep -q "$TEST_PACKAGE_AUR"; then
        pass "'$TEST_PACKAGE_AUR' binary available in PATH"
    else
        local bin_path
        bin_path=$(container_user_exec "which '$TEST_PACKAGE_AUR' 2>/dev/null" || true)
        skip "'$TEST_PACKAGE_AUR' binary not found in PATH (expected for bash scripts)"
    fi
...
test_export_hook() {
    ...
    if container_exec "test -x /usr/local/bin/distrobox-export-hook.sh"; then
    ...
    if container_exec "test -f /etc/pacman.d/hooks/99-distrobox-export.hook"; then
    ...
    local hook_out
    hook_out=$(timeout 30 distrobox_exec "env XDG_DATA_DIRS='/usr/local/share:/usr/share' XDG_DATA_HOME='/home/deck/.local/share' /usr/local/bin/distrobox-export-hook.sh 2>&1" || true)
...
test_icons() {
    ...
    for icon_path in \
        ...
        if ssh_check "test -f '$icon_path' 2>/dev/null"; then
...
test_pamac_manager_wrapper() {
    ...
    if container_exec "test -x /usr/local/bin/pamac-manager-wrapper"; then
    ...
    if container_exec "test -x /usr/local/bin/pamac-cli-wrapper"; then
    ...
    if container_exec "test -x /usr/local/bin/pamac-session-bootstrap.sh"; then
...
test_host_cli_wrapper() {
    ...
    if ssh_check "test -x '$wrapper_path'"; then
    ...
    local pamac_out
    pamac_out=$(timeout 30 ssh_check "'$wrapper_path' --version 2>&1" || true)
    ...
    local search_out
    search_out=$(timeout 30 ssh_check "'$wrapper_path' search neofetch 2>/dev/null" || true)
...
test_uninstall() {
    ...
    local remove_out
    remove_out=$(timeout 60 distrobox_exec "pamac remove --no-confirm --no-save '$TEST_PACKAGE_AUR' 2>&1" || true)
    ...
    local cleanup_out
    cleanup_out=$(timeout 30 distrobox_exec "pamac remove --no-confirm --unneeded 2>&1" || true)
...
test_db_integrity() {
    ...
    if container_exec "command -v yay >/dev/null 2>&1"; then
        pass "yay AUR helper still functional"
    fi

    if distrobox_exec "pamac list --aur 2>/dev/null | head -5" | grep -qi "$TEST_PACKAGE_AUR\|AUR"; then
...
test_host_integration() {
    ...
    if ssh_check "test -d /home/deck/.local/share/steamos-pamac/$CONTAINER_NAME"; then
    ...
    if ssh_check "test -f '$state_file'"; then
    ...
    local icon_cache_updated=false
    if ssh_check "test -f /home/deck/.local/share/icons/hicolor/icon-theme.cache 2>/dev/null"; then
        icon_cache_updated=true
    fi
    if ssh_check "test -f /home/deck/.local/share/icons/hicolor/scalable/apps/pamac-manager.svg 2>/dev/null"; then
        pass "Icon installed in hicolor scalable directory"
    fi
    if ! ssh_check "test -f /home/deck/.local/share/icons/hicolor/scalable/apps/pamac-manager.svg"; then
        if ssh_check "test -f /home/deck/.local/share/icons/hicolor/48x48/apps/pamac-manager.png"; then
            pass "Icon installed in hicolor 48x48 directory"
        fi
    fi

    if ! ssh_check "grep -q 'pamac-manager' /home/deck/.local/share/applications/arch-pamac-org.manjaro.pamac.manager.desktop 2>/dev/null"; then
