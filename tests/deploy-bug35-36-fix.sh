...
log "=== Bug #36 Fix: Suppress KDE's 'Uninstall or Manage Add-Ons...' menu item ==="
log "Strategy: Set NoDisplay=true appstream handler as user default."
log "KApplicationTrader::preferredService() skips NoDisplay services, so"
log "appstreamActions() returns empty list and the menu item won't appear."
...
log "Created NoDisplay appstream handler"
...
log "Set as default appstream handler in mimeapps.list"

update-desktop-database "$APP_DIR" 2>/dev/null
log "Updated desktop database"

log ""
log "=== Bug #35 Fix: kickeraction handler uses systemd-run --user --scope ==="
log "Strategy: KIO::ApplicationLauncherJob::start() is async (doesn't block Plasma)."
log "Handler also uses systemd-run --user --scope for extra safety so the uninstall"
log "process survives any parent process termination."
...
log "Updated kickeraction handler with systemd-run scope"

log ""
log "=== Update kickeraction desktop file ==="
log "X-KDE-OnlyForAppIds= means show for ALL apps (handler checks pamac marker internally)"
...
log "Updated kickeraction desktop file"

log ""
log "=== Clean up old appstream intercept ==="
rm -f "$APP_DIR/steamos-pamac-appstream-intercept.desktop"
rm -f "$BIN_DIR/steamos-pamac-appstream-intercept"
log "Removed old appstream intercept handler"

log ""
log "=== Verification ==="
log "appstream handler query:"
xdg-mime query default x-scheme-handler/appstream 2>&1
log ""
log "mimeapps.list:"
cat "$MIMEAPPS"
log ""
log "NoDisplay handler:"
cat "$APP_DIR/steamos-pamac-no-appstream-handler.desktop"
log ""
log "kickeraction desktop:"
cat "$KICKERACTION_DIR/steamos-pamac-uninstall.desktop"
log ""
log "kickeraction handler (first 5 lines):"
head -5 "$BIN_DIR/steamos-pamac-kickeraction-handler"
log ""
